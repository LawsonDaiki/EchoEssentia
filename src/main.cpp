...
void NTPConnect(void){
  Serial.print("Configurando relógio com NTP");
...
void checkWiFiThenMQTT(void){
  connectToWiFiVerbosely("Verificando WiFi");
  connectToMqtt();
}
...
void setup(){
  ...
  connectToWiFiVerbosely(String("Conectando a ") + String(WIFI_SSID));
...
void loop(){
  ...
  else
  {
    ...
    if (millis() - last_mueasurement_publish_in_millis > 5000)
    {
      ...
      // Print hold message into the LCD screen;
      Serial.println(".");
      lcd.setCursor(0, 0);
      lcd.print("Status:");
      lcd.setCursor(0, 1);
